...
export const getProjectTasks = asyncHandler(async (req, res) => {
  ...
  const whereClause = canSeeAllTasks
    ? { projectId }
  ...
  const tasks = await prisma.task.findMany({
    ...
    include: {
      ...
      predictions: {
        ...
        take: 1, // Get most recent prediction
        select: {
          id: true,
          predictedCompletionAt: true,
          confidence: true,
          factors: true,
        },
      },
...
export const updateTask = asyncHandler(async (req, res) => {
  ...
  // Validate HIGH priority requires at least 2 assignees total
  // ONLY if priority, assigneeId, or additionalAssigneeIds are being updated
  if (
    taskUpdates.priority !== undefined ||
...
export const approveTask = asyncHandler(async (req, res) => {
  ...
  if (task.status !== "IN_REVIEW") {
    return res.status(400).json({
      success: false,
      message: "Task must be in IN_REVIEW status to be approved",
    });
  }
